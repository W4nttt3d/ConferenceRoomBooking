...
/// <summary>Управління конференц-залами: CRUD і пошук доступних на заданий інтервал.</summary>
[ApiController]
...
public class ConferenceRoomsController : ControllerBase
{
    ...
    /// <response code="200">Список залів.</response>
    [HttpGet]
    ...
    /// </summary>
    /// <remarks>
    /// Межі інтервалів, що лише дотикаються, конфліктом не вважаються:
    /// зал, вільний з 10:00, можна знайти для запиту 06:00–10:00.
    /// </remarks>
    /// <param name="query">Бажаний інтервал часу та мінімальна місткість.</param>
    /// <response code="200">Список залів, вільних на заданий інтервал.</response>
    /// <response code="400">Некоректні параметри пошуку (наприклад, endTime раніше за startTime).</response>
    [HttpGet("available")]
    [ProducesResponseType(typeof(IReadOnlyList<ConferenceRoomResponse>), StatusCodes.Status200OK)]
    ...
    /// <response code="404">Зал з таким Id не існує.</response>
    [HttpGet("{id:int}")]
    ...
    /// <param name="request">Назва, місткість і базова погодинна ставка нового залу.</param>
    /// <response code="201">Зал створено; у заголовку Location — посилання на новий ресурс.</response>
    /// <response code="400">Дані не пройшли валідацію (наприклад, порожня назва або нульова місткість).</response>
    [HttpPost]
    ...
    /// <param name="id">Id залу.</param>
    /// <param name="request">Оновлені назва, місткість і базова погодинна ставка.</param>
    ...
    /// </summary>
    /// <param name="id">Id залу.</param>
    /// <response code="204">Зал деактивовано.</response>
    /// <response code="404">Зал з таким Id не існує.</response>
    [HttpDelete("{id:int}")]
